  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#000" }}>
      ...
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: padH, paddingTop: 18, paddingBottom: 14 }}>
        <Text style={{ color: "#fff", fontSize: 28, fontWeight: "800", letterSpacing: -0.5 }}>Founisè</Text>
        <Pressable
          onPress={onAdd}
          disabled={!onAdd}
      ...
      <View style={{ flexDirection: "row", alignItems: "center", gap: 10, paddingHorizontal: padH, marginBottom: 8 }}>
        ...
        <Pressable
          onPress={() => setShowBatchOnly(!showBatchOnly)}
          style={{
            width: 52, height: 52, borderRadius: 14, alignItems: "center", justifyContent: "center",
            backgroundColor: showBatchOnly ? "#fff" : "#000",
            borderWidth: 1, borderColor: showBatchOnly ? "#fff" : "#3a3a3c",
          }}
        >
          <Ionicons name="filter" size={20} color={showBatchOnly ? "#000" : "#fff"} />
        </Pressable>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          const contact = [item.phone, item.address].filter(Boolean).join(" | ");
          return (
            <Pressable onPress={() => setSelectedId(isSelected ? null : item.id)} style={{ backgroundColor: isSelected ? "#161616" : "#000" }}>
