  ...
  return (
    <>
      ...
      <View style={{ marginTop: 14, backgroundColor: palette.surface, borderRadius: radius.xl, padding: 16, borderWidth: 0.5, borderColor: palette.hairline, ...shadow.soft }}>
        <Text style={{ fontFamily: "Inter_400Regular", fontSize: 10, color: palette.muted2, fontWeight: "700", textTransform: "uppercase", letterSpacing: 0.7 }}>Repatisyon Peman</Text>
        <View style={{ gap: 8, marginTop: 10 }}>
          {payments.map(p => (
            <View key={p.label} style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
              ...
              <Text style={{ fontFamily: "Inter_400Regular", fontSize: 12, color: palette.muted2, width: 78 }}>{p.label}</Text>
              ...
              <Text style={{ fontFamily: "Inter_700Bold", fontWeight: "700", fontSize: 13, color: p.c, minWidth: 14, textAlign: "right" }}>{p.pct}%</Text>
            </View>
          ))}
        </View>
      ...
      <View style={{ flexDirection: "row", gap: 10, marginTop: 14 }}>
        <View style={{ flex: 1, backgroundColor: palette.blueBg, borderRadius: radius.lg, padding: 14, borderWidth: 0.5, borderColor: palette.blueBd, ...shadow.soft }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
            ...
            <Text style={{ fontFamily: "Inter_400Regular", fontSize: 9, color: palette.blue, fontWeight: "700", textTransform: "uppercase", letterSpacing: 0.5 }}>Tranzaksyon</Text>
          </View>
          <Text style={{ fontFamily: "Inter_700Bold", fontWeight: "700", fontSize: 24, color: palette.blue, marginTop: 6 }}>{filteredSalesCount}</Text>
          <Text style={{ fontFamily: "Inter_400Regular", fontSize: 11, color: palette.blue, marginTop: 2 }}>{rangeLabel} • live</Text>
        </View>
        <View style={{ flex: 1, backgroundColor: lowStockCount ? palette.warningBg : palette.successBg, borderRadius: radius.lg, padding: 14, borderWidth: 0.5, borderColor: lowStockCount ? palette.warningBd : palette.successBd, ...shadow.soft }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
            ...
            <Text style={{ fontFamily: "Inter_400Regular", fontSize: 9, color: lowStockCount ? palette.warning : palette.success, fontWeight: "700", textTransform: "uppercase", letterSpacing: 0.5 }}>Dezespwa</Text>
          </View>
          <Text style={{ fontFamily: "Inter_700Bold", fontWeight: "700", fontSize: 24, color: lowStockCount ? palette.warning : palette.success, marginTop: 6 }}>{lowStockCount}</Text>
          <Text style={{ fontFamily: "Inter_400Regular", fontSize: 11, color: lowStockCount ? palette.warning : palette.success, marginTop: 2 }}>{fmtG(lowStockValue)}</Text>
        </View>
      </View>
      ...
      <View style={{ marginTop: 14, backgroundColor: palette.surface, borderRadius: radius.xl, padding: 16, ...shadow.soft, borderWidth: 0.5, borderColor: palette.hairline }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 7 }}>
            ...
            <Text style={{ fontFamily: "Inter_700Bold", fontWeight: "700", fontSize: 13, color: palette.ink, letterSpacing: -0.2 }}>Tandans</Text>
          </View>
          <View style={{ paddingHorizontal: 9, paddingVertical: 4, borderRadius: radius.pill, backgroundColor: palette.surface2 }}>
            <Text style={{ fontFamily: "Inter_400Regular", fontSize: 10, color: palette.muted2, fontWeight: "600" }}>{rangeShort} • Top Vant</Text>
          </View>
        </View>
        <Text style={{ fontFamily: "Inter_400Regular", fontSize: 10, color: palette.muted2, marginTop: 2 }}>Seri plizyè KPI — kontribisyon chak pwodui nan revni a.</Text>
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8, marginTop: 12 }}>
          {topSeries.map(s => (
            <View key={s.name} style={{ flexDirection: "row", alignItems: "center", gap: 5 }}>
              ...
              <Text style={{ fontFamily: "Inter_400Regular", fontSize: 10, color: palette.muted2 }} numberOfLines={1}>{s.name}</Text>
            </View>
          ))}
        </View>
      </View>
      ...
      <View style={{ marginTop: 14, backgroundColor: palette.surface, borderRadius: radius.xl, overflow: "hidden", borderWidth: 0.5, borderColor: palette.hairline, ...shadow.soft }}>
        <View style={{ padding: 14, paddingBottom: 10, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          <Text style={{ fontFamily: "Inter_700Bold", fontWeight: "700", fontSize: 13 }}>Top Pwodui</Text>
          <View style={{ paddingHorizontal: 9, paddingVertical: 3, borderRadius: radius.pill, backgroundColor: palette.surfaceGrouped }}>
            <Text style={{ fontFamily: "Inter_400Regular", fontSize: 10, fontWeight: "600", color: palette.muted2 }}>TOP 5</Text>
          </View>
        </View>
          ...
          return (
            <View key={it.sku} style={{ padding: 12, flexDirection: "row", alignItems: "center", gap: 10, borderBottomWidth: idx === arr.length - 1 ? 0 : 0.5, borderBottomColor: palette.separatorSoft }}>
              <View style={{ width: 26, height: 26, borderRadius: 8, backgroundColor: rankStyle.bg, alignItems: "center", justifyContent: "center" }}>
                <Text style={{ fontFamily: "Inter_700Bold", fontSize: 10, fontWeight: "700", color: rankStyle.tint }}>#{it.rank}</Text>
              </View>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontFamily: "Inter_700Bold", fontWeight: "700", fontSize: 12, color: palette.ink }} numberOfLines={1}>{it.name}</Text>
                <Text style={{ fontFamily: "Inter_400Regular", fontSize: 10, color: palette.muted2, marginTop: 1 }}>{it.sku} • {it.qty} vann</Text>
              </View>
              <View style={{ alignItems: "flex-end" }}>
                <Text style={{ fontFamily: "Inter_700Bold", fontWeight: "700", fontSize: 12, color: palette.ink }}>{fmtG(it.amount)}</Text>
                <Text style={{ fontFamily: "Inter_400Regular", fontSize: 10, color: palette.muted2 }}>{(it.amount / it.qty).toFixed(0)} /u</Text>
              </View>
            </View>
          );
        })}
